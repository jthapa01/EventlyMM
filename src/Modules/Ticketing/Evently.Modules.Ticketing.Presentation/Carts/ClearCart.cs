...
internal sealed class ClearCart : IEndpoint
{
    ...
    {
        app.MapDelete("carts", async (ICustomerContext ICustomerContext, ISender sender) =>
        {
            Result result = await sender.Send(new ClearCartCommand(ICustomerContext.CustomerId));
